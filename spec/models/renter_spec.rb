...
RSpec.describe Renter, type: :model do
    it "is valid with valid attributes" do
        ...
        @renter = Renter.create(checkOutTime: Time.now, expectReturnTime: Time.now, returnTime: Time.now, suit_id: @suit.id, user_id: @user.id)
    ...
    it "is not valid without suit_id and uer_id" do
        ...
        :phone => "9876543210", :password => "123456", :password_confirmation => "123456")
        @suit =Suit.create(id: 1, appid: "good", size: "10", gender: "M", article: "fine", status: "Available")
        @renter = Renter.create(checkOutTime: Time.now, expectReturnTime: Time.now, returnTime: Time.now, suit_id: @suit.id, user_id: " ")
